...
class celeba_dataset(data.Dataset):
    ...
    def __init__(self, root, size):
        ...
        self.files = sorted(glob.glob(f"{root}/*.jpg"))[:dataset_sample_number]
...
def plot_random_gen(args, netG):
    ...
    scale = 0.5
    for i in range(1):
        sig_vals = np.array(sig_vals_each_class[i][:num_comp])
        var_vals = np.sqrt(sig_vals / np.sum(sig_vals))
        
        random_samples = np.random.normal(size=(nrow*nrow, num_comp))
        Z_random = means_each_class[i] + np.dot((scale * var_vals * random_samples), components_each_class[i][:num_comp]) # can modify scale to lower value to get more clear results  
        ...
        plt.imshow(np.transpose(vutils.make_grid(X_recon_random[:nrow*nrow], nrow=nrow, padding=2, normalize=True).cpu(), (1,2,0)))

        save_dir = os.path.join(args.model_dir, 'figures', 'images')
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        file_name = os.path.join(save_dir, f"random_recon_images_epoch{args.epoch}_scale{scale}_class{i}_ncomp{num_comp}_nrow{nrow}_norm.png")
        plt.savefig(file_name)
        print("Plot saved to: {}".format(file_name))
    ...
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    file_name = os.path.join(save_dir, f"random_recon_images_epoch{args.epoch}_scale{scale}_ncomp{num_comp}_nrow{nrow}_all_norm.png")
    plt.savefig(file_name)
    print("Plot saved to: {}".format(file_name))
...
def plot_recon_imgs(train_X, train_X_bar, test_X=None, test_X_bar=None):
    ...
    plt.imshow(np.transpose(vutils.make_grid(train_X[:nrow*nrow], nrow=nrow, padding=2, normalize=True).cpu(), (1,2,0)))
    ...
    plt.imshow(np.transpose(vutils.make_grid(train_X_bar[:nrow*nrow], nrow=nrow, padding=2, normalize=True).cpu(), (1,2,0)))
...
def plot_nearest_samples_transform_celebA(args, features, epoch, traindata, netG, netD):
    ...
    # img_id_1 = 0
    sample_num = 10
    img_id_1 = np.random.randint(0, dataset_sample_number, 10)
    feature_dim = features.shape[1]
    ...
    print("Plot saved to: {}".format(file_name))
        ...
        # fig, ax = plt.subplots(ncols=lin_sample_num+1, nrows=1, figsize=(50, 20))
        # for c in range(lin_sample_num+1):
        #     ax[c].imshow(np.moveaxis(x_lin_sample[c].numpy(), 0, -1))
        #     ax[c].set_xticks([])
        #     ax[c].set_yticks([])
        #     ax[c].spines['top'].set_visible(False)
        #     ax[c].spines['right'].set_visible(False)
        #     ax[c].spines['bottom'].set_linewidth(False)
        #     ax[c].spines['left'].set_linewidth(False)
        # file_name = os.path.join(save_dir, f"lin_sample_images_{img_id_1}.png")
        # fig.savefig(file_name)
        # print("Plot saved to: {}".format(file_name))
        # plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dir', default='./log/mcrgan_celebA_binary/', type=str, help='base directory for saving PyTorch model.')
    ...
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    log_dir = './log/mcrgan_celebA_binary/'
    ## load model
    netG = sngan.SNGANGenerator128().to(device)
    netD = customSNGANDiscriminator128().to(device)

    state_dict_G = torch.load(log_dir + 'netG_100000_steps.pth')['model_state_dict']
    state_dict_D = torch.load(log_dir + 'netD_100000_steps.pth')['model_state_dict']
    ...
    # load dataset
    data_root = "/home/dataset/celeba/img_align_celeba"
    ...
    print(train_X.max(), train_X.min(), train_X_bar.max(), train_X_bar.min())
    print(train_Z.norm(dim=1, p=2), train_Z_bar.norm(dim=1, p=2))
